...
This fork is already installed at `~/.claude/skills/pstack/`, which Claude Code auto-loads as the `pstack@skills-dir` plugin. Confirm with:

```bash
claude plugin list | grep -A2 pstack
```

To turn it off later: `claude plugin disable pstack`. To pull upstream changes, see [`PORT-NOTES.md`](../../PORT-NOTES.md).
...
The model file is read at delegation time, so it takes effect immediately.
