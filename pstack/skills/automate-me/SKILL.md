...
Locate the active workspace's transcripts before fanning out. Use only `~/.claude/projects/<slug>/`, where `<slug>` is the cwd with each "/" turned into "-". Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
