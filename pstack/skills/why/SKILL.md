...
Before spawning investigators, list the available MCP servers in this session (the tool list, or `ToolSearch` over `mcp__` names). Skip a source whose MCP is absent and say so.
...
- give it full tool access. `general-purpose` reaches MCP servers through `ToolSearch`; a read-only agent type such as `Explore` cannot, which disables MCP-backed investigators entirely. The source control investigator would be safe in readonly, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
- give it full tool access. The synthesizer's quality check spot-verifies citations, which can require MCP access, so do not use a read-only agent type.
